...
// 	faces, _, err := w.grpcClient.DetectFaces(frameData, width, height, 1) // 1 channel for encoded data
// 	if err != nil {
...
func (w *IngestWorker) processVideoFrame(frameData []byte, width, height, frameCount int) {
	...
	faces, _, err := w.grpcClient.DetectFaces(dummyFrame, width, height, 1) // 1 channel for grayscale
	if err != nil {
		log.Printf("❌ Face detection error: %v", err)
		return
	}
